...
bool Parser::load(const std::string &str) {
    str_   = str;
    index_ = 0;
...
Xml Parser::parse() {
    ...
    if (str_.compare(index_, 5, "<?xml") == 0) {
        if (!parseDeclaration()) throw std::logic_error("parse declaration error");
    }

    // 识别注释
    skipWhiteSpace();
    if (str_.compare(index_, 4, "<!--") == 0) {
        if (!parseComment()) throw std::logic_error("parse comment error");
...
bool Parser::parseDeclaration() {
    ...
    if (str_.compare(index_, 5, "<?xml") != 0) return false;
...
bool Parser::parseComment() {
    ...
    if (str_.compare(index_, 4, "<!--") != 0) return false;
    ...
    while (str_[index_] != '\0') {
        ...
        if (str_[index_] == '/') {
            ...
            if (str_[index_ + 1] == '/') {
                ...
                // 解析注释
                if (!parseComment()) throw std::logic_error("xml comment is error");
            } else {
                // 解析子节点
            ...
            if (str_[index_] != '=') throw std::logic_error("xml element attr is error: " + key);
...
std::string Parser::parseElementName() {
    ...
    if (isalpha(str_[index_]) || str_[index_] == '_') {
        index_++;
        while (isalnum(str_[index_]) || (str_[index_] == ':') || (str_[index_] == '-') || (str_[index_] == '.')) {
            index_++;
        }
    }
...
std::string Parser::parseElementAttrKey() {
    ...
    if (isalpha(str_[index_]) || (str_[index_] == '_')) {
        index_++;
        while (isalnum(str_[index_]) || (str_[index_] == '_') || (str_[index_] == '-') || (str_[index_] == ':')) {
            index_++;
        }
    }
...
std::string Parser::parseElementAttrValue() {
    ...
    // 判断属性值是否以双引号开始
    if (str_[index_] != '"') throw std::logic_error("xml element attr value should be in double quotes");
